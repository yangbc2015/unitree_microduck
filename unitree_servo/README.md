...
| `bam_unit.py` | **多颗舵机的批量流程**：`run`（一颗跑完整电池→自包含记录）/ `compare`（跨单元对比+重复性）/ `limit`（M4 限流） |
| `docs/S288_个体差异测试方法.md` | **个体差异测试方法**：先立噪声地板、判据表、产出怎么读、测不到什么 |
...
- S288 摩擦约为 XL330 的 **1.6×（库仑）/ 1.9×（粘滞）**。**下凹的形状别当真**：同一天两次
  背靠背拟合给出 vs 0.0095 vs 0.025、alpha 3.5 vs 0.5（都贴边界），实测带内的 F(v) 只差
  ~1.5 mN·m —— 用稳健值（破断/平台/最低点/粘滞斜率），不用形状参数。见附录 B。
...
## 附录 B：动力学测量（M2/M6b/M7，2026-09-11 实测）

详细记录和原始数据见 `bam_data/S288_dynamics_测量记录.md`。四项里有三项**不需要额外设备**
且已完成，第四项被物理条件挡住：

| 项 | 结果 | 怎么测的 |
|----|------|---------|
| 指令延迟 | **0.48 ms** 传输 + 一阶力矩建立 τ=**0.54 ms**（10–90% 1.19 ms） | 力矩阶跃 ±0.02 N·m（**低于破断点→轴不动**，隔离出纯通信+建立时间），80 次跃变全点拟合；残差 2.26 counts |
| 传动链刚度 | **测不可靠，不给值** | 破断以下 ±τ 扫描：重复时输出端编码器先动 4.5 mrad（反作用/蠕滑，方向与转子通道相反），工具判 NOT MEASURED |
| 慢速蠕动起点 | 0.028–0.032 N·m（对比快速破断 0.0365） | 同一扫描，轴开始蠕动的力矩 |
| **回差** | **1.056° = 0.0184 rad**（三位置 0.0177–0.0192，重复性 0.014°） | 每个位置**分别从下方/上方逼近**、1.8 s 停稳后测量 |
| 编码器间恒定偏置 | −1.63°（随位置 ±0.4°，含输出编码器非线性） | 同上；这是标定偏置，不是回差 |
| **关节等效惯量** | **5e-4 kg·m²（4–6e-4）**，转子等效 6.1e-9 | 力矩阶跃 0.05/0.06/0.07 N·m，只取 α 最大的窗口 |

三个要点：

1. **延迟必须拆成"传输 + 上升"两段**。手册预期 "<5 ms"，实测端到端 0.48 ms（还含 USB-CDC
   和主机调度）；sim 里光加纯延迟不够，还得有 ~0.5 ms 一阶惯性。
2. **"输出端编码器消除反冲"不成立**：实测 1.06° 的输出端滞回。对足式机器人这不是小量。
   （第一版粗略测量看到的 0.5–1.7° 就是这个现象，当时以为是编码器偏差。）
3. **惯量测出来了，而且是 5e-4 而不是 0.1**：关键在于 `--vmax`（把分析限制在摩擦模型的拟合区间内，
   否则轴在几毫秒内冲到 13 rad/s，等于用外推的摩擦算 J），以及只采信 α 最大的窗口
   （α→0 的窗口 `J=(τ−F)/α` 数学上发散，实测 0.005–0.0066）。机身用手按住前后 4.0e-4 vs 4.7e-4
   没有变化 —— 所以之前"机身滑转导致测不出"的判断被否掉了，那个 0.1 是建立在"转子惯量 1.2e-6"
   这个我从未测量的假设上；手册自己的堵转数据（0.32 A→3.4e-4 N·m）给出 kt≈1e-3 N·m/A，正是
   小转子高转速电机的特征。交叉验证：与 microduck 发布的 XL330 `armature=0.00157`（同为 288:1，
   关节空间）同量级。
