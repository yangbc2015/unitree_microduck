...
3. **机身自由时测不出惯量，这是物理限制**：反作用力矩 ≈ 输出端力矩（20–120 mN·m），而让轴
   动起来本身就需要 ≥ 摩擦 25 mN·m —— 所以任何运动都意味着反作用超过台面能抓住的量级，
   机身必然滑转。两种独立方法都给出 ~5e-4 kg·m²，比"1.2e-6 kg·m² 转子 × 288² = 0.1"小两个
   数量级，且力矩与速度相关(+0.67)、与加速度不相关(−0.03)。**需要先把舵机刚性夹持再重跑**：
   `python3 bam_dynamics.py inertia --taus 0.06,0.09,0.12`（判据：J 应升到 ~0.1 量级）。
